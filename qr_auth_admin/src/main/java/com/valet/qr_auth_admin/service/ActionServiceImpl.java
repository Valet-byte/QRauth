...
public class ActionServiceImpl implements ActionService {
    ...
    public boolean changePassword(Long userId, String userEmail, String password) {
        try {
            ...
            emailService.sendSimpleEmail(userEmail, "Change password", "http://" + host + ":9630/do/" + action.getId());
            return true;
        } catch (Exception e){
            e.printStackTrace();
            return false;
    ...
    public boolean changeJobTitle(Long userId, String userEmail, String jobTitle) {
        try {
            ...
            emailService.sendSimpleEmail(userEmail, "Change job_title", "http://" + host + ":9630/do/" + action.getId());
            return true;
        } catch (Exception e){
            e.printStackTrace();
            return false;
    ...
    public boolean changeName(Long userId, String userEmail, String name) {
        try {
            ...
            emailService.sendSimpleEmail(userEmail, "Change username", "http://" + host + ":9630/do/" + action.getId());
            return true;
        } catch (Exception e){
            e.printStackTrace();
            return false;
